    ...
    if person and is_pin_valid(person, pin_code):
        while True:
            choice = int(input('Выберите пункт:\n'
                               ...
                               'Ваш выбор:'))
            if choice == 1:
                print('Ваш баланс составляет ', person['money'])
            if choice == 2:
                loan = float(input('Какую сумму вы желаете снять? '))
                if loan > person['money']:
                    print('На вашем счету не достаточно средств для снятия')
                if loan < 0:
                    print('Пожалуйста, введите положительное число')
                else:
                    hlp = person['money']
                    person.pop('money')
                    person['money'] = "%.2f" %(hlp - loan)
...
try:
    ...
    print('Вводите пожалуйста только цифры, использование любых других знаков недопустимо')